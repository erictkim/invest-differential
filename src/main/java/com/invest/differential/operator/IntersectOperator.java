...
/**
 * Incremental INTERSECT operator.
 *
 * Maintains accumulated left and right inputs. On each step:
 * 1. Adds deltas to accumulated state
 * 2. Computes intersect(left, right) — rows present in both sides
 * 3. Diffs with previous output to produce delta
 */
public final class IntersectOperator implements Operator {
    ...
    public IntersectOperator(Stream left, Stream right, BufferAllocator allocator) {
        this.left = left;
        this.right = right;
    ...
    public void step() {
        ...
        // Compute intersect: rows in both left and right
        ZSet currentResult = accumulatedLeft.intersect(accumulatedRight);
